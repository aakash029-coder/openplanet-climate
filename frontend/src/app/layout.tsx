...
export default function RootLayout({
  ...
  return (
    <html lang="en">
      {/* ── IMAGE DIRECTLY ON BODY (IMPOSSIBLE TO HIDE) ── */}
      <body 
        className="font-mono min-h-screen flex flex-col text-white selection:bg-indigo-500/30 bg-cover bg-center bg-fixed bg-no-repeat"
        style={{
          backgroundImage: "url('/satellite-map.jpeg')", 
          backgroundColor: "#020617" // Fallback sirf tab dikhega agar image path galat hoga
        }}
      >
        <Providers>
          <Navbar />
          
          {/* Main content overlay so the text is readable */}
