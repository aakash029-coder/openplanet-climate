...
export default function DashboardPage() {
  ...
  // ✅ FIX: Resize debounce ref — har pixel pe modal check nahi hoga
  const resizeTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    const checkScreenSize = () => {
      // ✅ Already dismissed? Kabhi dobara mat dikhao — even after resize
    ...
    // Initial check — mount pe ek baar
    checkScreenSize();

    // ✅ FIX: Debounced resize — 100ms wait, phir check
    // Pehle: har pixel scroll/rotate pe fire hota tha
    // Ab: 100ms gap ke baad ek baar check
    const handleResize = () => {
  ...
  // ✅ Tab visit memory — dependency array correct, preserve
  useEffect(() => {
    setVisitedTabs((prev) => {
  ...
  return (
    <div className="relative text-slate-200 font-sans overflow-x-hidden flex flex-col w-full min-h-screen">
      ...
      {/* ✅ FIX: Background image GPU-accelerated — scroll pe stable */}
      <img
        src="/cybermap.jpeg"
      ...
      <div className="flex flex-col w-full flex-grow relative z-10">
        ...
        <div className="w-full h-full flex flex-col flex-grow">
          ...
          {/* DASHBOARD TAB — hide instead of destroy (keep-alive) */}
          ...
          <div className={activeTab !== "Dashboard" ? "max-w-[1400px] w-full mx-auto px-6 md:px-12 py-16" : "hidden"}>
            ...
            {/* COMPARE — sirf tab mount hoga jab visited ho aur city ho */}
            ...
            {/* RESEARCH — sirf tab mount hoga jab visited ho aur city ho */}
            ...
            {/* METHODOLOGY — city required nahi */}
