...
function MobileWorkspace() {
  ...
  // — Current CMIP6 projection from context ——
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      if (searchQuery.length > 2 && !selectedCity) {
        ...
        } catch { /* Nominatim timeout — silent */ }
      } else {
        setSuggestions([]);
      }
    }, 600);
  ...
  // — Fit bounds after hex data loads ——
  useEffect(() => {
  ...
  return (
    <div className="w-full flex flex-col min-h-screen" style={{ background: 'var(--canvas)' }}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5"
        ...
        <span className="font-mono text-[8px]" style={{ color: 'var(--muted)', opacity: 0.45 }}>
          CMIP6 · ERA5
        </span>
      </div>
      ...
      <div className="px-4 pt-3 pb-3 flex flex-col gap-2"
        ...
        <div className="relative">
          ...
          <input
            ...
            style={{
              ...
              color: 'var(--text)', height: '40px', padding: '0 12px 0 36px',
            }}
          />
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <select
            ...
            style={{
              background: 'rgba(255,255,255,0.03)', border: '1px solid var(--hairline)',
              color: 'var(--text)', height: '38px', padding: '0 24px 0 10px',
        ...
        {/* Primary CTA — white block button */}
        <button
      ...
      >
        ...
        {/* Zoom controls — 32px touch targets */}
        ...
        {/* Legend — compact, text-[9px], tight padding */}
        {isInitialized && (
          <div className="absolute bottom-2 left-2 z-50 px-1.5 py-1"
               style={{ background: 'rgba(8,8,10,0.95)', border: '1px solid var(--hairline)' }}>
            <p className="font-mono text-[9px] uppercase tracking-[0.09em] mb-0.5" style={{ color: 'var(--muted)' }}>
              {year} · {ssp}
            </p>
      ...
      {/* ── INDICATORS — dense 2×2 grid ── */}
      {isInitialized && (
        <div className="grid grid-cols-2"
             style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          {([
            {
              label:    'Heat Deaths / yr',
              value:    isSimulating ? mitDeaths : deaths,
              baseline: isSimulating ? deaths     : null,
              color:    isSimulating ? 'var(--positive)' : 'var(--heat-4)',
              sub:      '±15% CI · Gasparrini 2017',
            },
            {
              label:    'Economic Loss / yr',
              value:    isSimulating ? mitLoss : loss,
              baseline: isSimulating ? loss    : null,
              color:    isSimulating ? 'var(--positive)' : 'var(--heat-2)',
              sub:      '±8% CI · Burke 2018',
            },
            {
              label:    'Heatwave Days',
              value:    isSimulating ? `${mitHW}d`   : `${heatwave}d`,
              baseline: isSimulating ? `${heatwave}d` : null,
              color:    isSimulating ? 'var(--positive)' : 'var(--heat-3)',
              sub:      'Above P95 · CMIP6',
            },
            {
              label:    'Peak Tx5d',
              value:    isSimulating ? `${mitTemp}°C` : `${temp}°C`,
              baseline: isSimulating ? `${temp}°C`    : null,
              color:    isSimulating ? 'var(--positive)' : 'var(--copper)',
              sub:      'ERA5 decadal mean',
            },
          ] as const).map((m, i) => (
            <div
              key={m.label}
            ...
            >
              ...
              {m.baseline !== null && (
                <p className="font-mono text-[8px] tabular-nums"
              ...
              <p className="font-mono text-[7px]"
                 style={{ color: 'var(--muted)', opacity: 0.45 }}>
                {m.sub}
              </p>
            </div>
      ...
      {isInitialized && (
        <div className="px-4 py-3 flex flex-col gap-2.5"
          ...
          <div className="flex items-center gap-1.5">
            <div className="w-1 h-1 rounded-full" style={{ background: 'var(--reference)' }} />
            <span className="font-mono text-[8px] uppercase tracking-[0.16em]"
                  style={{ color: 'var(--reference)' }}>
              Adaptation Scenario
            </span>
            <span className="font-mono text-[7px] italic" style={{ color: 'var(--muted)' }}>
              — directional only
            </span>
          </div>
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center">
              <label className="font-mono text-[8px] uppercase tracking-wider" style={{ color: 'var(--muted)' }}>
                Canopy Cover
              </label>
              <span className="font-mono text-[9px] font-bold tabular-nums" style={{ color: 'var(--positive)' }}>
                +{canopy}%
              </span>
            </div>
            <div className="relative">
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center">
              ...
              <span className="font-mono text-[9px] font-bold tabular-nums" style={{ color: 'var(--reference)' }}>
                +{coolRoof}%
              </span>
            </div>
            <div className="relative">
...
// ── Exported wrapper (ClimateDataProvider + navbar clearance) ────────────────
