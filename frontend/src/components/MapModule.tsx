    ...
    const delayDebounceFn = setTimeout(async () => {
      if (searchQuery.length > 2 && !selectedCity) {
        ...
        try {
          ...
          console.error("Geocoding Error Bypass:", err);
        } finally {
    ...
    <div className="w-full flex flex-col relative z-0">
      ...
      <section className="relative w-full h-[750px] bg-[#020617] overflow-hidden border-b border-slate-800">
        ...
        {isInitialized && !isLoading && !apiError && (
          <div className="absolute bottom-[90px] left-1/2 -translate-x-1/2 z-40 flex flex-col items-center animate-bounce pointer-events-none drop-shadow-lg">
            <span className="text-[10px] font-mono text-indigo-400 uppercase tracking-[0.2em] mb-1 font-bold">Scroll to Explore</span>
            <div className="w-3 h-3 border-b-2 border-r-2 border-indigo-400 rotate-45"></div>
          </div>
        )}
                      ...
                      {suggestions.map((city, idx) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            setSearchQuery(`${city.name}, ${city.admin1 ? city.admin1 + ', ' : ''}${city.country}`);
                            setSuggestions([]);
                          }}
              ...
              <>
                ...
                <div className="bg-[#0a0f1d] border border-slate-800 p-5 rounded-md relative shadow-inner overflow-visible">
                  <div className="flex items-center justify-between mb-2 relative overflow-visible">
                    ...
                    <InfoTooltip alignLeft publicText="Financial capital loss projected from heat-induced productivity drops and infrastructure damage." techText="GDP Loss Fraction Model." />
                  </div>
                  <div className="text-3xl font-mono text-slate-200 tracking-tight">{val(simData.loss)}</div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[#0a0f1d] border border-slate-800 p-4 rounded-md relative shadow-inner flex flex-col justify-between overflow-visible">
