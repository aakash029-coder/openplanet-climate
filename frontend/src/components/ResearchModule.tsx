...
function cleanResearchText(t: string | null): string {
  if (!t) return "";
  return t.replace(/\*/g,'').replace(/([a-z])([.?!])([A-Z])/g,'$1$2 $3').replace(/\beuros?\b/gi,'USD').replace(/€/g,'$');
}
...
const CalcModal = ({ open, onClose, auditSection, title, disclaimer }: {
  open:boolean; onClose:()=>void; auditSection:any; title:string; disclaimer:string;
}) => {
...
// ═══════════════════════════════════════════════════════════════
// SECTION 1: SURVIVABILITY TIMELINE
// ═══════════════════════════════════════════════════════════════
const SurvivabilityTimeline = ({ projections }: { projections: Projection[] }) => {
  const zones = [
    { label:"Safe",    bg:"bg-emerald-500", color:"#10b981" },
    { label:"Danger",  bg:"bg-orange-500",  color:"#f97316" },
    { label:"Critical",bg:"bg-red-500",     color:"#ef4444" },
    { label:"Fatal",   bg:"bg-red-900",     color:"#7f1d1d" },
  ];
  return (
    <div className="bg-[#050814] border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.03]" style={{backgroundImage:'radial-gradient(circle at 20% 50%, #ef4444 0%, transparent 60%), radial-gradient(circle at 80% 50%, #f97316 0%, transparent 60%)'}}/>
      <div className="relative">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <div>
            <h3 className="text-[11px] font-mono text-slate-300 uppercase tracking-[0.3em] font-bold">Survivability Timeline</h3>
            <p className="text-[9px] font-mono text-slate-600 mt-1 italic">When does this city cross the physiological limit? · Real CMIP6 WBT data</p>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            {zones.map(z=>(
              <div key={z.label} className="flex items-center gap-1.5">
                <div className={`w-2 h-2 rounded-full ${z.bg}`}/>
                <span className="text-[8px] font-mono text-slate-500 uppercase">{z.label}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="space-y-3">
          {projections.map(p=>{
            const wbt  = p.wbt_max_c ?? 0;
            const pct  = Math.min(100,(wbt/40)*100);
            const zone = wbt>=35?zones[3]:wbt>=31?zones[2]:wbt>=28?zones[1]:zones[0];
            return (
              <div key={p.year}>
                <div className="flex items-center gap-4 mb-1">
                  <span className="text-[10px] font-mono text-slate-400 w-12 shrink-0">{p.year}</span>
                  <div className="flex-grow relative h-8 bg-slate-900/60 rounded-lg overflow-hidden border border-slate-800/60">
                    <div className="absolute inset-0 flex pointer-events-none">
                      <div className="h-full bg-emerald-500/10" style={{width:'70%'}}/>
                      <div className="h-full bg-orange-500/10" style={{width:'7.5%'}}/>
                      <div className="h-full bg-red-500/10"    style={{width:'10%'}}/>
                      <div className="h-full bg-red-900/20"    style={{width:'12.5%'}}/>
                    </div>
                    <div className="absolute inset-y-0 left-0 flex items-center justify-end pr-2 transition-all duration-700"
                      style={{width:`${pct}%`,background:`linear-gradient(90deg,#1e293b,${zone.color})`}}>
                      <span className="text-[9px] font-mono text-white font-bold">{fmt(wbt)}°C</span>
                    </div>
                    {[{p:70,l:'28°C'},{p:77.5,l:'31°C'},{p:87.5,l:'35°C'}].map(m=>(
                      <div key={m.p} className="absolute inset-y-0 border-l border-dashed border-white/10" style={{left:`${m.p}%`}}>
                        <span className="absolute top-0 -translate-x-1/2 text-[7px] font-mono text-slate-700">{m.l}</span>
                      </div>
                    ))}
                  </div>
                  <div className="shrink-0 w-20 text-right">
                    <span className={`text-[9px] font-mono font-bold uppercase ${zone.color==='#10b981'?'text-emerald-400':zone.color==='#f97316'?'text-orange-400':zone.color==='#ef4444'?'text-red-400':'text-red-700'}`}>{zone.label}</span>
                  </div>
                </div>
                <div className="flex gap-2 ml-16 mb-1 flex-wrap">
                  {wbt>=28&&wbt<31 && <span className="text-[7px] font-mono bg-orange-500/10 border border-orange-500/20 text-orange-400 px-1.5 py-0.5 rounded">⚠ Outdoor work unsafe</span>}
                  {wbt>=31&&wbt<35 && <span className="text-[7px] font-mono bg-red-500/10 border border-red-500/20 text-red-400 px-1.5 py-0.5 rounded">☠ Unprotected exposure fatal</span>}
                  {wbt>=35         && <span className="text-[7px] font-mono bg-red-900/20 border border-red-900/30 text-red-700 px-1.5 py-0.5 rounded">☣ Beyond human physiological limit</span>}
                </div>
              </div>
            );
          })}
        </div>
        <div className="mt-5 pt-4 border-t border-slate-800/60 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            {threshold:28,label:'Danger crossed',  color:'text-orange-400',border:'border-orange-500/20',bg:'bg-orange-950/20'},
            {threshold:31,label:'Critical crossed', color:'text-red-400',   border:'border-red-500/20',   bg:'bg-red-950/20'  },
            {threshold:35,label:'Fatal limit',      color:'text-red-700',   border:'border-red-900/20',   bg:'bg-red-950/30'  },
          ].map(t=>{
            const first = projections.find(p=>(p.wbt_max_c??0)>=t.threshold);
            return (
              <div key={t.threshold} className={`${t.bg} border ${t.border} rounded-xl p-3 text-center`}>
                <p className="text-[8px] font-mono text-slate-500 uppercase tracking-widest mb-1">{t.label}</p>
                {first
                  ? <><p className={`text-lg font-mono font-bold ${t.color}`}>{first.year}</p><p className="text-[8px] font-mono text-slate-600 italic mt-0.5">WBT {fmt(first.wbt_max_c)}°C</p></>
                  : <p className="text-sm font-mono text-slate-700">Not projected</p>
                }
              </div>
            );
          })}
        </div>
        <SourceLine source="Stull (2011) WBT formula · CMIP6 via Open-Meteo · Sherwood & Huber (2010) PNAS thresholds"/>
      </div>
    </div>
  );
};
...
// ═══════════════════════════════════════════════════════════════
// SECTION 2: CLIMATE DEBT CALCULATOR
// ═══════════════════════════════════════════════════════════════
const ClimateDebtCalculator = ({ projections, canopy, albedo }: {
  projections: Projection[]; canopy: number; albedo: number;
}) => {
  const [discountRate, setDiscountRate] = useState(3);
  const [actionYear, setActionYear]     = useState(2030);
  const cooling  = (canopy/100)*1.2 + (albedo/100)*0.8;
  const allYears = [2030,2040,2050,2060,2070,2080,2090,2100];

  const getInterp = (year: number, key: 'economic_decay_usd'|'attributable_deaths'): number => {
    const sorted = [...projections].sort((a,b)=>a.year-b.year);
    const before = sorted.filter(p=>p.year<=year).pop();
    const after  = sorted.find(p=>p.year>year);
    if (!before) return projections[0]?.[key]??0;
    if (!after)  return before[key];
    const t = (year-before.year)/(after.year-before.year);
    return before[key]+t*(after[key]-before[key]);
  };

  let baseDebt=0, mitDebt=0, baseDeaths=0, mitDeaths=0;
  allYears.forEach((yr,i)=>{
    const loss   = getInterp(yr,'economic_decay_usd');
    const deaths = getInterp(yr,'attributable_deaths');
    const r      = 1/Math.pow(1+discountRate/100,i);
    const hwBase = projections.find(p=>p.year>=yr)?.heatwave_days ?? projections[projections.length-1]?.heatwave_days ?? 30;
    const effHW  = Math.max(0,hwBase-cooling*3.5);
    const hwR    = hwBase>0?effHW/hwBase:1;
    const mR     = yr>=actionYear ? hwR*Math.max(0,1-cooling*0.08) : 1;
    baseDebt   += loss*r; mitDebt   += loss*mR*r;
    baseDeaths += deaths; mitDeaths += deaths*mR;
  });
  const saved      = baseDebt-mitDebt;
  const livesSaved = Math.round(baseDeaths-mitDeaths);
  const savingsPct = baseDebt>0?(saved/baseDebt)*100:0;
  const barMax     = Math.max(baseDebt,mitDebt);
    ...
    <div className="bg-[#050814] border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
      ...
      <div className="relative">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
          <div>
            <h3 className="text-[11px] font-mono text-slate-300 uppercase tracking-[0.3em] font-bold">Climate Debt Calculator</h3>
            <p className="text-[9px] font-mono text-slate-600 mt-1 italic">Cumulative NPV economic loss 2030–2100 · Real CMIP6 anchors · Intermediate years interpolated</p>
          </div>
          <div className="flex items-center gap-5 shrink-0">
            <div className="space-y-1">
              <div className="flex justify-between gap-3">
                <label className="text-[9px] font-mono text-slate-500 uppercase">Discount Rate</label>
                <span className="text-[9px] font-mono text-amber-400">{discountRate}%</span>
              </div>
              <input type="range" min={1} max={8} step={0.5} value={discountRate} onChange={e=>setDiscountRate(Number(e.target.value))} className="w-28 accent-amber-500 cursor-pointer" style={{touchAction:'manipulation'}}/>
            </div>
            <div className="space-y-1">
              <div className="flex justify-between gap-3">
                <label className="text-[9px] font-mono text-slate-500 uppercase">Action Year</label>
                <span className="text-[9px] font-mono text-cyan-400">{actionYear}</span>
              </div>
              <input type="range" min={2030} max={2075} step={5} value={actionYear} onChange={e=>setActionYear(Number(e.target.value))} className="w-28 accent-cyan-500 cursor-pointer" style={{touchAction:'manipulation'}}/>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          {[
            ...
            <div key={m.label} className={`${m.bg} border ${m.border} rounded-2xl p-4 text-center`}>
              <p className="text-[8px] font-mono text-slate-500 uppercase tracking-widest mb-2">{m.label}</p>
        ...
        <div className="space-y-2">
          {allYears.map(yr=>{
            const loss    = getInterp(yr,'economic_decay_usd');
            const hwBase  = projections.find(p=>p.year>=yr)?.heatwave_days??30;
            const effHW   = Math.max(0,hwBase-cooling*3.5);
            const hwR     = hwBase>0?effHW/hwBase:1;
            const mR      = yr>=actionYear?hwR*Math.max(0,1-cooling*0.08):1;
            const mitLoss = loss*mR;
            const isReal  = projections.some(p=>p.year===yr);
            const isAction= yr===actionYear;
            return (
              <div key={yr} className="flex items-center gap-3">
                <div className="flex items-center gap-1.5 w-16 shrink-0">
                  <span className={`text-[9px] font-mono ${isAction?'text-cyan-400 font-bold':isReal?'text-slate-300':'text-slate-600'}`}>{yr}</span>
                  <div className={`w-1.5 h-1.5 rounded-full ${isReal?'bg-cyan-500':'bg-slate-700'}`} title={isReal?'Real API data':'Interpolated'}/>
                </div>
                <div className="flex-grow space-y-1">
                  <div className="h-3 bg-slate-900 rounded overflow-hidden relative">
                    <div className="h-full bg-red-500/60 rounded transition-all duration-500" style={{width:`${(loss/barMax)*100}%`}}/>
                    <span className="absolute inset-y-0 right-2 flex items-center text-[7px] font-mono text-white/40">{fmtUSD(loss)}</span>
                  </div>
                  {(canopy>0||albedo>0)&&(
                    <div className="h-3 bg-slate-900 rounded overflow-hidden relative">
                      <div className="h-full bg-emerald-500/60 rounded transition-all duration-500" style={{width:`${(mitLoss/barMax)*100}%`}}/>
                      <span className="absolute inset-y-0 right-2 flex items-center text-[7px] font-mono text-white/40">{fmtUSD(mitLoss)}</span>
                    </div>
                  )}
                </div>
                {isAction&&<span className="text-[7px] font-mono text-cyan-400 border border-cyan-500/30 bg-cyan-950/30 px-1.5 py-0.5 rounded shrink-0">ACTION</span>}
              </div>
            );
          })}
          <div className="flex items-center gap-5 pt-2 flex-wrap">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-sm bg-red-500/60"/><span className="text-[8px] font-mono text-slate-500">No Action</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-sm bg-emerald-500/60"/><span className="text-[8px] font-mono text-slate-500">With Mitigation</span></div>
            <div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-cyan-500"/><span className="text-[8px] font-mono text-slate-600">Real API point</span></div>
            <div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-slate-700"/><span className="text-[8px] font-mono text-slate-600">Interpolated</span></div>
          </div>
        </div>
        <SourceLine source="Burke et al. (2018) · ILO (2019) · Standard NPV discounting"/>
        <EstimateDisclaimer text="Intermediate years linearly interpolated between real CMIP6 API anchor points. For research and scenario planning only — not a financial instrument."/>
      </div>
    </div>
  );
};
...
// ═══════════════════════════════════════════════════════════════
// SECTION 3: ADAPTATION ROI ENGINE
// ═══════════════════════════════════════════════════════════════
const AdaptationROI = ({ selectedProj, gdp_usd, population, canopy, albedo, setCanopy, setAlbedo }: {
  ...
  const cooling     = (canopy/100)*1.2+(albedo/100)*0.8;
  const hwR         = selectedProj.heatwave_days>0 ? Math.max(0,selectedProj.heatwave_days-cooling*3.5)/selectedProj.heatwave_days : 1;
      ...
      <div className="relative">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-[9px] font-mono text-slate-600 mt-1 italic">Investment cost vs economic return — 30-year horizon · Use sliders or presets below</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            {presets.map(p=>(
              <button key={p.label} onClick={()=>{setCanopy(p.canopy);setAlbedo(p.albedo);}}
                className={`px-3 py-1.5 rounded-lg text-[8px] font-mono uppercase tracking-widest border transition-all ${canopy===p.canopy&&albedo===p.albedo?'border-emerald-500/50 bg-emerald-950/30 text-emerald-300':'border-slate-800 text-slate-500 hover:border-slate-700 hover:text-slate-300'}`}>
                {p.label}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          <div className="bg-slate-900/30 border border-slate-800/60 rounded-xl p-4">
            <p className="text-[9px] font-mono text-slate-400 uppercase tracking-widest mb-3">Investment Breakdown <span className="text-slate-600 normal-case">(order-of-magnitude est.)</span></p>
            {[
              ...
              <div key={item.label} className="mb-3">
                ...
                <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                  <div className={`h-full ${item.color} rounded-full transition-all duration-500`} style={{width:totalInvest>0?`${(item.cost/totalInvest)*100}%`:'0%'}}/>
                </div>
          ...
          <div className="bg-slate-900/30 border border-slate-800/60 rounded-xl p-4">
            <p className="text-[9px] font-mono text-slate-400 uppercase tracking-widest mb-3">30-Year Benefit <span className="text-slate-600 normal-case">(real API × 30yr)</span></p>
            <div className="space-y-3">
              {[
                ...
                <div key={item.label} className="flex justify-between items-center border-b border-slate-800/40 pb-2 last:border-0">
                  ...
                  <span className={`text-[13px] font-mono font-bold ${'highlight' in item?(item.highlight?'text-cyan-400':'text-red-400'):'text-emerald-400'}`}>{item.val}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-slate-900/20 border border-slate-800/40 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-[11px] font-mono font-bold ${roiColor}`}>{totalInvest>0?`${fmt(roi,0)}% return`:'Set sliders above to calculate'}</span>
          </div>
          <div className="h-3 bg-slate-900 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700" style={{width:`${Math.min(100,Math.max(0,roi/5))}%`,background:'linear-gradient(90deg,#f97316,#10b981,#22d3ee)'}}/>
          </div>
...
// ═══════════════════════════════════════════════════════════════
// SECTION 4: HEAT MORTALITY AGE DECOMPOSITION
// ═══════════════════════════════════════════════════════════════
const MortalityDecomposition = ({ selectedProj, population, onCalcClick }: {
      ...
      <div className="relative">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-[9px] font-mono text-slate-600 mt-1 italic">Vulnerability-weighted attribution by demographic cohort · GBD 2019 multipliers</p>
          </div>
          <div className="flex bg-slate-900/60 rounded-xl p-1 border border-slate-800/60 shrink-0">
            {(['deaths','factors','interventions'] as const).map(t=>(
              <button key={t} onClick={()=>setTab(t)}
                className={`px-3 py-1.5 rounded-lg text-[8px] font-mono uppercase tracking-widest transition-all ${tab===t?'bg-slate-700 text-white':'text-slate-500 hover:text-slate-300'}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-5 p-4 bg-red-950/20 border border-red-500/20 rounded-xl">
          ...
          <div className="flex-grow min-w-[160px]">
            ...
            <div className="h-4 flex rounded-full overflow-hidden">
              {withDeaths.map(c=>(
                <div key={c.label} className={`${c.barColor} transition-all duration-700`} style={{width:`${c.deathPct}%`}} title={`${c.label}: ${c.deaths.toLocaleString()}`}/>
              ))}
            </div>
          ...
          {/* Calc button here — for mortality formula */}
          ...
          {withDeaths.map(c=>(
            <div key={c.label} className={`${c.bg} border ${c.border} rounded-2xl p-4`}>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-[7px] font-mono border ${c.border} ${c.bg} ${c.textColor} px-1.5 py-0.5 rounded-full uppercase font-bold`}>{c.risk}</span>
              </div>
              ...
              {tab==='deaths'&&(
                <>
                  ...
                  <div className="space-y-2">
                    <div>
                      <div className="flex justify-between text-[8px] font-mono text-slate-500 mb-1"><span>Relative vulnerability</span><span>{c.vuln}×</span></div>
                      <div className="h-2 bg-slate-900 rounded-full overflow-hidden"><div className={`h-full ${c.barColor} rounded-full`} style={{width:`${Math.min(100,(c.vuln/3.5)*100)}%`}}/></div>
                    </div>
                    <div>
                      <div className="flex justify-between text-[8px] font-mono text-slate-500 mb-1"><span>Share of burden</span><span>{fmt(c.deathPct,1)}%</span></div>
                      <div className="h-2 bg-slate-900 rounded-full overflow-hidden"><div className={`h-full ${c.barColor} opacity-60 rounded-full`} style={{width:`${c.deathPct}%`}}/></div>
                    </div>
                  </div>
                </>
              )}
        ...
        <div className="bg-slate-900/30 border border-slate-800/40 rounded-xl p-3 mt-3">
          <p className="text-[8px] font-mono text-slate-600 leading-relaxed">
            <span className="text-slate-500 font-bold">Methodology:</span> Total deaths ({totalDeaths.toLocaleString()}) from real Gasparrini (2017) API engine. Per-cohort distribution applies GBD 2019 vulnerability multipliers to UN WUP (2022) population shares, normalized to match API total exactly. Individual cohort figures are research estimates.
          </p>
        </div>
...
export default function ResearchModule({ baseTarget }: { baseTarget: string }) {
  ...
  // Single modal state — 'economics' | 'mortality'
  ...
  const handleAnalyse = useCallback(async (queryToRun:string=baseTarget, sspVal=ssp)=>{
    ...
    try {
      ...
      let riskRetries=3,riskData:any=null,lastRiskError:any=null;
      ...
      if (riskData.projections?.length>0) {
        ...
        let aiRetries=3,aiSuccess=false;
  ...
  const getMitigatedData = () => {
    ...
    return {
      ...
      loss:mitLoss, deaths:mitDeaths, savedDeaths,
      savedLoss:sp.economic_decay_usd-mitLoss,
      combinedRatio:combined, hwDays:Math.round(effHW),
    };
  };
  ...
  const excelData: ExcelExportData|null = (result&&selectedProj&&geo)?{
    ...
    mean_temp_c:selectedProj.peak_tx5d_c-8,population:result.population??0,
    gdp_usd:result.gdp_usd??0,death_rate:7.7,
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-700">
      ...
      {/* Calc Modals */}
      <CalcModal open={calcModal.open&&calcModal.key==='mortality'} onClose={()=>setCalcModal(m=>({...m,open:false}))} auditSection={mortalityAudit} title="Mortality" disclaimer="Simplified Gasparrini et al. 2017 — for research estimation only."/>
      <CalcModal open={calcModal.open&&calcModal.key==='economics'} onClose={()=>setCalcModal(m=>({...m,open:false}))} auditSection={economicsAudit} title="Economics" disclaimer="Simplified Burke (2018) + ILO (2019) framework — for estimation only."/>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end border-b border-slate-800 pb-5 gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-mono font-bold text-white uppercase tracking-tighter truncate max-w-2xl">{baseTarget.split(',')[0]}</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          {result.projections.map(p=>(
            <button key={p.year} onClick={()=>setSelectedYear(p.year)}
              ...
              {p.year}<span className="ml-2 text-[8px] text-slate-600">{p.source.includes('cmip6')?'CMIP6':'AR6'}</span>
            </button>
          ))}
        </div>
      )}
      ...
      {!loading&&result&&dynamicData&&selectedProj&&(
        <>
          {/* ── BASELINE vs MITIGATION (only when sliders active) ── */}
          {hasMitigation&&(
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={item.label} className="space-y-2">
                    ...
                    <div className="flex items-baseline justify-between"><span className="text-[8px] font-mono text-slate-600">Without</span><span className={`text-[13px] font-mono font-bold tabular-nums ${item.baseColor}`}>{item.baseline}</span></div>
                    <div className="flex items-baseline justify-between"><span className="text-[8px] font-mono text-slate-600">With</span><span className="text-[13px] font-mono font-bold tabular-nums text-slate-300">{item.mitigated}</span></div>
                    <div className="flex items-center justify-between bg-emerald-950/25 rounded-lg px-2.5 py-1.5 border border-emerald-800/20"><span className="text-[8px] font-mono text-slate-600">Saved</span><span className="text-[11px] font-mono text-emerald-400 font-bold">{item.saved}</span></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="bg-[#050814] border border-slate-800 p-5 rounded-xl flex flex-col">
              ...
              <div className={`flex flex-col items-center justify-center py-8 border-y border-slate-800/50 my-4 flex-grow rounded-lg ${wbtStatus.bg}`}>
                ...
                <div className={`mt-3 px-4 py-1 rounded-full border ${wbtStatus.border} ${wbtStatus.color} ${wbtStatus.bg} text-[9px] font-mono font-bold`}>{wbtStatus.label}</div>
                {hasMitigation&&<p className="text-[9px] font-mono text-slate-600 mt-3 italic">Baseline: {formatWBT(selectedProj.wbt_max_c??0)}</p>}
              </div>
              <p className="text-[9px] font-mono text-slate-500 leading-relaxed">Values ≥ 31°C critical. Capped at 35°C per Sherwood & Huber (2010) PNAS.</p>
              {result.baseline?.baseline_mean_c&&<p className="text-[8px] font-mono text-slate-600 italic mt-2">ERA5 baseline: {fmt(result.baseline.baseline_mean_c)}°C (1991-2020)</p>}
            ...
            <div className="bg-[#050814] border border-slate-800 p-5 rounded-xl flex flex-col">
              ...
              <div className="space-y-5 flex-grow">
                {[
                  ...
                  <div key={i}>
                    <div className="flex justify-between text-[10px] font-mono mb-1.5"><span className="text-slate-500 uppercase">{item.label}</span><span className={item.color}>{item.val}</span></div>
                    <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden"><div className={`${item.bar} h-full transition-all duration-500`} style={{width:item.w}}/></div>
            ...
            <div className="bg-[#050814] border border-slate-800 p-5 rounded-xl flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 flex-grow content-start">
                ...
                <div className="p-4 bg-white/5 border border-white/5 rounded-lg flex flex-col justify-center">
                  ...
                  <span className={`text-xl font-mono font-bold ${dynamicData.peakTemp>38?'text-amber-400':'text-emerald-400'}`}>{dynamicData.peakTemp>38?'HIGH':'LOW'}</span>
                  <span className="text-[8px] font-mono text-slate-600 block mt-1 uppercase">38°C+ Exposure</span>
                </div>
              </div>
              <div className="mt-5 space-y-2.5">
                <div className="flex items-center gap-3 text-[10px] font-mono text-slate-400 uppercase"><div className={`w-1.5 h-1.5 rounded-full ${dynamicData.cdd>500?'bg-red-500 animate-pulse':'bg-emerald-500'}`}/><span>Thermal Overload: {dynamicData.cdd>500?'Exceeded':'Stable'}</span></div>
                <div className="flex items-center gap-3 text-[10px] font-mono text-slate-400 uppercase"><div className={`w-1.5 h-1.5 rounded-full ${selectedProj.wbt_max_c&&selectedProj.wbt_max_c>=31?'bg-red-500 animate-pulse':'bg-emerald-500'}`}/><span>Survivability: {selectedProj.wbt_max_c&&selectedProj.wbt_max_c>=31?'Critical':'Stable'}</span></div>
              </div>
              <SourceLine source="CMIP6 Ensemble · ERA5 P95"/>
            </div>
          </div>

          {/* ── ECONOMIC CARD (full width) ── */}
          <div className="bg-indigo-600/5 border border-indigo-500/20 p-5 rounded-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[10px] font-mono text-indigo-400 uppercase tracking-widest font-bold">Economic Value At Risk ({selectedYear})</h3>
              <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <div><span className="text-[9px] font-mono text-slate-500 uppercase block">City GDP</span><span className="text-base font-mono font-bold text-slate-400">{fmtUSD(result.gdp_usd)}</span></div>
                  <div><span className="text-[9px] font-mono text-slate-500 uppercase block">Population</span><span className="text-base font-mono font-bold text-slate-400">{result.population?(result.population/1e6).toFixed(1)+'M':'—'}</span></div>
                </div>
              ...
              <div>
                ...
                <div className="h-24 flex items-end gap-1 w-full border-b border-slate-700 pb-1">
                  {result.projections.map((p,idx)=>{
                    ...
                    return (
                      <div key={p.year} className="bg-red-500 w-full cursor-pointer hover:bg-red-400 rounded-t-sm transition-all duration-500 relative group"
                        style={{height:`${Math.min(100,(loss/maxLoss)*100)}%`,opacity:0.4+idx*0.2}}
                        onClick={()=>setSelectedYear(p.year)}>
                        <div className="absolute -top-6 left-1/2 -translate-x-1/2 hidden group-hover:block bg-slate-800 text-white text-[7px] font-mono px-1.5 py-0.5 rounded whitespace-nowrap z-10">{p.year}: {fmtUSD(loss)}</div>
                      </div>
                    );
                  })}
                </div>
                <div className="flex justify-between text-[8px] font-mono text-slate-600 uppercase mt-1">{result.projections.map(p=><span key={p.year}>{p.year}</span>)}</div>
              </div>
            </div>
          </div>

          {/* ── 4 UNIQUE DEEP DIVE SECTIONS ── */}
          <SurvivabilityTimeline projections={result.projections}/>

          <ClimateDebtCalculator
